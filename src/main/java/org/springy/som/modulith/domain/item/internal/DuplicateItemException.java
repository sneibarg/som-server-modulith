package org.springy.som.modulith.domain.item.internal;

import org.springframework.http.HttpStatus;
import org.springy.som.modulith.exception.ApiError;
import org.springy.som.modulith.exception.ConflictApiException;

@ApiError(status = HttpStatus.CONFLICT, title = "Duplicate item vnum", code = "DUPLICATE_ITEM_VNUM")
public final class DuplicateItemException extends ConflictApiException {
    public DuplicateItemException(String message) {
        super(message);
    }
}

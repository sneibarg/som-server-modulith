...
public class MobileControllerTest {
    ...
    void getMobileById_blankId_becomes400ProblemDetail() throws Exception {
        when(mobileService.getMobileById(anyString())).thenThrow(new InvalidMobileException("Item id must be provided"));
    ...
    void createMobile_returns201() throws Exception {
        ...
        Mobile saved = new Mobile();
        saved.setAreaId("A1");
        saved.setId("I1");
        saved.setName("Midgaard");

        when(mobileService.createMobile(any(Mobile.class))).thenReturn(saved);
        ...
        verify(mobileService).createMobile(any(Mobile.class));
    }

...
public class ResetControllerTest {
    ...
    void createReset_returns201() throws Exception {
        Reset input = new Reset();
        input.setId("I1");
        input.setAreaId("A1");

        Reset saved = new Reset();
        saved.setId("I1");
        saved.setAreaId("A1");
        

        when(resetService.createReset(any(Reset.class))).thenReturn(saved);
        ...
        verify(resetService).createReset(any(Reset.class));
    }
